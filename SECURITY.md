...
Huddle is an employee agent that turns a meeting recap into deal-team follow-through: action items, open decisions, and a digest.
...
| Control                                         | Count |
| ----------------------------------------------- | ----- |
| Production Apex classes                         | 12    |
| Classes declared `with sharing`                 | 11    |
| Classes declared `without sharing`              | 0     |
| Production SOQL queries                         | 28    |
| Queries carrying an explicit enforcement clause | 28    |
| Queries deliberately running in system mode     | 0     |
| Dynamic queries (`Database.query`)              | 1     |
| Dynamic queries passing `AccessLevel.USER_MODE` | 1     |
| Apex test methods                               | 42    |

Every production query carries an explicit enforcement clause. There are no
exceptions.
...
**Writes are gated on explicit human confirmation.** Action items and record updates are proposed first and written only on explicit confirmation.
...
- It does not email anyone. Digests are written to Salesforce records.
- It does not call any external service.
- It does not infer commitments that are absent from the recap it was given.
